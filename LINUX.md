...
| `tests/test_wine_post_message.py` | **new** — 64 tests: the Steam/Proton parsing against a fabricated tree, the protocol against a real loopback server, and the backend's semantics |
...
| `ok/util/process.py` | `is_admin()` answers `os.geteuid() == 0` on Linux, so the `Pynput` fallback stops reporting a privilege problem it does not have |
...
Baseline: **555 passed, 6 failed, 1 skipped, 16 subtests passed** (562 collected, Python
3.12) — 376 of those passes predate Phase 2 (`tests/test_x11_window.py`, 74), Phase 3
(`tests/test_x11_capture.py`, 41) and Phase 4 (`tests/test_wine_post_message.py`, 64).
